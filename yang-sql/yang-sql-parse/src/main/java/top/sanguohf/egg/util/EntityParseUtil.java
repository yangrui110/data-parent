...
public class EntityParseUtil {
    ...
    public static EntityJoinTable parseViewEntityTable(Class entity) throws ClassNotFoundException, NoSuchFieldException {
        ...
        boolean dictinct = entity.isAnnotationPresent(Distinct.class);
        selectSql.setDistinct(dictinct);
        if(present) {
            //设置表别名
        ...
        //设值分组
        List<EntityGroupBy> groupByList = ReflectEntity.collectDefaultGroupBy(entity);
        selectSql.setGroupBys(groupByList);
        return selectSql;
    }
