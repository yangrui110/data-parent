...
public class ReflectEntity {
     ...
     * 只查出主键的列
     * */
    public static Map<String,Object> reflectPrimaryKeys(Class entity){
        try {
            ...
            throw new RuntimeException("类错误", e);
        }
        ...
        try {
            for (Field field : fields) {
                if (field.isAnnotationPresent(Id.class)) {
                    ...
                    result.put(tableField, field.getName());
                }
            }
        }catch (Exception e){
            throw new RuntimeException("获取主键属性错误", e);
        }
        if(result.size()>0)
            return result;
        else {
            ...
            try {
                ...
                result.put(tableField,"id");
            }catch (NoSuchFieldException|ClassNotFoundException ex){
                throw new RuntimeException("主键不存在或者未设置", ex);
            }
    ...
    //获取到
    public static List<EntityColumn> getTotalColumns(Class entity){
        List<EntityColumn> columnList = new LinkedList<>();
        List<Field> fields = getFields(entity);
        for(Field field1: fields){
            top.sanguohf.egg.annotation.Field fs=field1.getAnnotation(top.sanguohf.egg.annotation.Field.class);
     ...
     * 收集到待排序的字段
     * */
    public static List<EntityGroupBy> collectDefaultGroupBy(Class entity) throws NoSuchFieldException, ClassNotFoundException {
        List<Field> fields = getFields(entity);
        LinkedList<EntityGroupBy> list = new LinkedList<>();
        if(entity.isAnnotationPresent(GroupBy.class)){
            GroupBy annotation = (GroupBy)entity.getAnnotation(GroupBy.class);
            list.add(getOneGroupBy(annotation,entity,null));
        }
        for(Field field:fields){
            if(field.isAnnotationPresent(GroupBy.class)){
                GroupBy annotation = field.getAnnotation(GroupBy.class);
                list.add(getOneGroupBy(annotation,entity,field));
            }else if(field.isAnnotationPresent(OrderBys.class)){
                GroupBys orderBys = field.getAnnotation(GroupBys.class);
                for(GroupBy orderBy:orderBys.value()){
                    list.add(getOneGroupBy(orderBy,entity,field));
                }
            }
        }
        return list;
    ...
    //获取到单个ordeyBy数据
    private static EntityGroupBy getOneGroupBy(GroupBy groupBy,Class entity,Field field) throws NoSuchFieldException, ClassNotFoundException {

        EntityGroupBy entityOrderBy = new EntityGroupBy();
        String tableField = "";
        ...
        if(present) {
            if (field != null){
                ...
                tableField = getTableField(forName, groupBy.column());
                fieldColumnName = groupBy.column();
            }else {
                String tableALias = groupBy.tableAlias();
            }
        } else {
            tableField = getTableField(entity, field.getName());
    ...
    public static List<EntityColumn> collectColumns(List<EntityColumn> entityColumns,String[] includeCloumns,String[] excludeColumns,String tableAlias) throws NoSuchFieldException {
        ...
        if((includeCloumns.length==0)&&excludeColumns.length==0){
            ...
            for(String column:includeCloumns){
                String substring = getFirstColumn(column);
                String columnAlias = getRealColumnAlias(column);
                //
                for(EntityColumn entityColumn:entityColumns){
                    if(substring.equals(entityColumn.getFieldName())){
                        entityColumn.setAliasColumn(columnAlias);
    ...
    private static String getFirstColumn(String string){
    ...
    //传入字符串形如：column columnAlias或者column as columnAlias
    private static String getRealColumnAlias(String column){
        //截取到开始点到空格的字符串
        int indexOf = column.indexOf(" ");
        String substring = column.trim();
        String columnAlias = null;
        if(indexOf!=-1){
            columnAlias = column.substring(column.lastIndexOf(" ")).trim();
        }else columnAlias = substring;
        return columnAlias;
    }
    //获取到视图表需要查询出的列
